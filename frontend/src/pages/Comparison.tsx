...
import { GitCompare, Upload, Calendar, X, ArrowLeftRight, ZoomIn, ZoomOut, RotateCcw, Loader2 } from "lucide-react";
...
const isTiffFile = (file: File): boolean => {
    ...
    return extension === 'tif' || extension === 'tiff' || file.type === 'image/tiff';
};
    ...
    return new Promise((resolve, reject) => {
        ...
        reader.onload = async (e) => {
            try {
                ...
                if (ifds.length === 0) {
                    reject(new Error('No TIFF images found in file'));
                    return;
                }
...
const Comparison = () => {
    ...
    const [isLoading, setIsLoading] = useState<{ left: boolean; right: boolean }>({ left: false, right: false });
        ...
        return async (event: React.ChangeEvent<HTMLInputElement>) => {
            ...
            if (file) {
                try {
                    ...
                    // Check if file is TIFF and convert it
                    if (isTiffFile(file)) {
                        imageUrl = await convertTiffToDataUrl(file);
                    } else {
                    ...
                    if (side === "left") {
                        if (leftImage && !leftImage.url.startsWith('data:')) URL.revokeObjectURL(leftImage.url);
                        setLeftImage(newImage);
                    } else {
                        if (rightImage && !rightImage.url.startsWith('data:')) URL.revokeObjectURL(rightImage.url);
    ...
    const handleDrop = useCallback((side: "left" | "right") => {
        return async (event: React.DragEvent) => {
            ...
            // Check for image files including TIFF
            const isImageOrTiff = file && (file.type.startsWith("image/") || isTiffFile(file));

            if (file && isImageOrTiff) {
                try {
                    setIsLoading(prev => ({ ...prev, [side]: true }));
        ...
        <div className="sharp-card" style={{ flex: 1, padding: "1.5rem", minHeight: "400px", display: "flex", flexDirection: "column" }}>
            ...
            {isLoading[side] ? (
                <div style={{
                    ...
                    <Loader2 style={{ width: "48px", height: "48px", color: "#8b5cf6", animation: "spin 1s linear infinite" }} />
                    <p style={{ color: "#94a3b8" }}>Processing TIFF image...</p>
                </div>
            ) : image ? (
                ...
                >
                    ...
                    <div style={{ textAlign: "center" }}>
                        ...
                        <span style={{ color: "#64748b", fontSize: "0.875rem" }}>Supports: PNG, JPEG, TIFF</span>
                    </div>
                    <label style={{
