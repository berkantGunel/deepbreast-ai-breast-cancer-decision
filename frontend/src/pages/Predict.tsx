  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (isDicomFile(file)) {
        ...
        setIsDicom(false);
        try {
          const tiffDataUrl = await convertTiffToDataUrl(file);
          setPreview(tiffDataUrl);
        } catch (err) {
          setPreview(null);
          console.error("TIFF preview failed:", err);
          setError("Failed to load TIFF image. Please try a different file.");
        }
      } else {
        setIsDicom(false);
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
          ...
          <div className="sharp-card">
            ...
            <label style={{ cursor: "pointer", display: "block" }}>
              ...
              <input type="file" style={{ display: "none" }} accept="image/*,.tif,.tiff,.dcm,.dicom,application/dicom" onChange={handleFileSelect} />
            </label>
          </div>
