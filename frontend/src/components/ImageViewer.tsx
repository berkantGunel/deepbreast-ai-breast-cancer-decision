...
const ImageViewer = ({ src, alt = 'Medical Image', className = '' }: ImageViewerProps) => {
    ...
    // Wheel zoom handler - uses native event for proper preventDefault
    ...
    useEffect(() => {
        const canvas = containerRef.current?.querySelector('.image-viewer-canvas');
        if (canvas) {
            canvas.addEventListener('wheel', handleWheelZoom as EventListener, { passive: false });
            return () => {
                canvas.removeEventListener('wheel', handleWheelZoom as EventListener);
            };
        }
    }, [handleWheelZoom]);
