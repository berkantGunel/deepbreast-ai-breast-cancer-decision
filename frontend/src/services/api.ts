...
export interface MammographyHealthResponse {
  ...
  models: {
    pathology_model: boolean;
    tissue_model: boolean;
    abnormality_model: boolean;
  };
...
const transformMammographyResponse = (response: MammographyPredictionResponse): MammographyPredictionResponse => {
  const pathology = response.pathology_classification;
  const risk = response.risk_assessment;

  // Map pathology to prediction name
  let prediction: string;
  let urgency: "low" | "medium" | "high";

  if (pathology.class === 'M' || pathology.is_malignant) {
    prediction = 'Malignant';
    urgency = 'high';
  } else if (pathology.class === 'B') {
    prediction = 'Benign';
    urgency = 'low';
  } else {
    prediction = 'Normal';
  ...
  // Get probabilities
  const probabilities = {
    benign: (pathology.probabilities['B'] || 0) * 100,
    suspicious: (pathology.probabilities['N'] || 0) * 100,  // Normal as "suspicious" for UI
    malignant: (pathology.probabilities['M'] || 0) * 100
  };
  ...
  const riskMap: Record<string, string> = {
    'Critical': 'BI-RADS 5',
    'High': 'BI-RADS 4',
    'Medium-High': 'BI-RADS 4',
    'Medium': 'BI-RADS 3',
    'Low-Medium': 'BI-RADS 2',
    'Low': 'BI-RADS 1'
  };
  ...
  return {
    ...response,
    prediction,
    confidence: pathology.confidence * 100,
    birads_category: riskMap[risk.level] || 'BI-RADS 3',
    probabilities,
    recommendation: {
      action: risk.recommendation,
      urgency,
      description: `${pathology.name} - ${risk.level} risk`,
      next_steps: getNextSteps(risk.level, pathology.name)
    }
  };
};

const getNextSteps = (riskLevel: string, pathologyName: string): string[] => {
  if (riskLevel === 'Critical' || pathologyName === 'Malignant') {
    return [
  ...
  } else if (riskLevel === 'High' || riskLevel === 'Medium-High') {
    return [
      'Additional diagnostic imaging recommended',
      'Consider ultrasound or MRI',
      'Biopsy may be indicated',
      'Consult with radiologist'
    ];
  } else if (pathologyName === 'Benign') {
    return [
      'Continue regular mammography screening',
      'Follow-up in 6-12 months recommended',
      'No immediate intervention required'
    ];
  }
  return [
    'Continue regular screening program',
...
export const predictMammography = async (
  ...
  return transformMammographyResponse(response.data);
};

// Get mammography info
export const getMammographyInfo = async (): Promise<MammographyInfoResponse> => {
  const response = await api.get<MammographyInfoResponse>("/mammography/classical/info");
  return response.data;
};

// Check mammography health
export const getMammographyHealth = async (): Promise<MammographyHealthResponse> => {
  const response = await api.get<MammographyHealthResponse>("/mammography/classical/health");
  return response.data;
};

export interface MammographyPreviewResponse {
  success: boolean;
...
// Get mammography preview (handling TIFF etc)
export const getMammographyPreview = async (file: File): Promise<MammographyPreviewResponse> => {
  const formData = new FormData();
  formData.append("file", file);

  const response = await api.post<MammographyPreviewResponse>(
    "/mammography/classical/preview",
    formData,
    {
      headers: { "Content-Type": "multipart/form-data" },
    }
